...
describe("loadConfig", () => {
  ...
  it("requires production-only values together", () => {
    assert.throws(
      () => loadConfig({
        NODE_ENV: "production",
  ...
  it("requires stripe secrets when Stripe is enabled", () => {
    assert.throws(
      () => loadConfig({
        ...baseEnv,
        PAYMENTS_PROVIDER: "stripe"
  ...
  it("rejects non-strict integers", () => {
    assert.throws(
      () => loadConfig({
        ...baseEnv,
        PORT: "1.5",
        WORKER_CONCURRENCY: "12x"
      }),
      (error) => {
        assert.match(error.message, /PORT must be a strict integer/);
        assert.match(error.message, /WORKER_CONCURRENCY must be a strict integer/);
        return true;
      }
    );
  });
  ...
  it("rejects empty list entries", () => {
    assert.throws(
      () => loadConfig({
        ...baseEnv,
        FEATURE_FLAGS: "a,,b",
        JOB_QUEUES: "email,,reports"
      }),
      (error) => {
        assert.match(error.message, /FEATURE_FLAGS(?: item must have length >= 1|\[1\] is required)/);
        assert.match(error.message, /JOB_QUEUES(?: item must have length >= 1|\[1\] is required)/);
        return true;
      }
    );
  });
