...
describe("loadConfig", () => {
  ...
  it("requires production-only values together", () => {
    assert.throws(
      ...
      (error) => {
        ...
        assert.match(error.message, /SESSION_SECRET is required in production/);
        return true;
      }
    );
  });
  ...
  it("rejects non-strict integers", () => {
    assert.throws(
      ...
      (error) => {
        ...
        assert.match(error.message, /WORKER_CONCURRENCY must be an integer/);
        return true;
      }
    );
  });
  ...
  it("rejects empty list entries", () => {
    assert.throws(
      ...
      (error) => {
        assert.match(error.message, /FEATURE_FLAGS must not include empty items/);
        assert.match(error.message, /JOB_QUEUES must not include empty items/);
        return true;
      }
    );
  });
