...
const EnvSchema = v.object({
  NODE_ENV: v.picklist(ENVIRONMENTS),
  APP_NAME: v.string(),
  HOST: v.string(),
  PORT: intSchema(1, 65535),
  PUBLIC_URL: urlSchema(),
  TRUST_PROXY: boolSchema(),
  SUPPORT_EMAIL: v.pipe(v.string(), v.check((value) => value.includes("@"), "must include @")),
  ...
  LOG_FORMAT: v.picklist(LOG_FORMATS),
  ...
  PAYMENTS_PROVIDER: v.picklist(PAYMENT_PROVIDERS),
...
export function loadEnv(env = process.env) {
  ...
  const errors = result.success ? [] : result.issues.map(formatIssue);
  const values = result.success ? result.output : fallbackValues(input);
...
function intSchema(min, max) {
  return v.pipe(
    v.string(),
    v.check((value) => /^[+-]?\d+$/.test(value), "must be an integer"),
    v.transform(Number),
    v.check((value) => value >= min, `must be >= ${min}`),
    v.check((value) => value <= max, `must be <= ${max}`)
  );
}
...
function boolSchema() {
  return v.pipe(
    ...
    v.check((value) => typeof value === "boolean", "must be a boolean")
  );
}
...
function urlSchema() {
  return v.pipe(
    v.string(),
    v.check((value) => !empty(value), "is required"),
    v.transform((value) => parseUrl(value)),
    v.check((value) => value !== undefined, "must be a URL")
  );
}

function optionalUrlSchema() {
  return v.optional(v.pipe(v.string(), v.transform((value) => parseUrl(value)), v.check((value) => value !== undefined, "must be a URL")));
}
...
function listSchema() {
  return v.pipe(
    v.string(),
    v.transform((value) => empty(value) ? [] : value.split(",").map((item) => item.trim())),
    v.check((list) => list.every((item) => item !== ""), "must not include empty items")
  );
}
...
function urlListSchema() {
  return v.pipe(
    v.string(),
    v.check((value) => !empty(value), "is required"),
    v.transform((value) => value.split(",").map((item) => parseUrl(item.trim()))),
    v.check((list) => list.every((item) => item !== undefined), "item must be a URL")
  );
}

function jsonSchema() {
  return v.pipe(v.string(), v.transform((value) => {
    try {
      return JSON.parse(value);
    } catch {
      return undefined;
    }
  }), v.check((value) => value !== undefined, "must be valid JSON"));
}
...
function formatIssue(issue) {
  const key = issue.path?.[0]?.key || "value";
